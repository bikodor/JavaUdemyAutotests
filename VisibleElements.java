...
public class VisibleElements {
    ...
    public void visibleElement(){
        ...
        driver.switchTo().frame("frame_examples"); //если фрейм, то переходим вот так


        WebElement buttonOk = driver.findElement(By.id("okButton"));

        JavascriptExecutor jse = (JavascriptExecutor)driver;
        jse.executeScript("arguments[0].scrollIntoView()", buttonOk); //скроллим к кнопке


        JavascriptExecutor executor = (JavascriptExecutor)driver;
        executor.executeScript("arguments[0].click();", buttonOk); // в некоторых случаях кликать через js необходимо
